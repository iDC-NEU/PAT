...
// ATTENTION: when adding new variables adapt maxEntries
   uint32_t datastructureIds {0};
   uint64_t numEntries = 0;
   static constexpr uint64_t maxEntries = (EFFECTIVE_PAGE_SIZE - (sizeof(uint32_t) + sizeof(uint64_t)  + sizeof(uint64_t) + sizeof(bool))) / (sizeof(CatalogEntry));
