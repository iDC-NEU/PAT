...
namespace scalestore
{
   ...
   struct Timer
   {
      ...
      std::chrono::nanoseconds local_total_duration{0};  // 累计的时间，初始化为0微秒
      std::chrono::nanoseconds remote_total_duration{0}; // 累计的时间，初始化为0微秒
      ...
      void stop(bool local)
      {
         if (running)
         {
            ...
            if (local)
            {
               local_total_duration += std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time);
            }
            else
            {
               remote_total_duration += std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time);
            }
            running = false;
         }
      }
      ...
      void reset(bool clear_total = false)
      {
         if (clear_total)
         {
            local_total_duration = std::chrono::microseconds(0);
            remote_total_duration = std::chrono::microseconds(0);
         }
         start_time = std::chrono::high_resolution_clock::now();
         running = false;
      }
      ...
      // 获取累计的时间（微秒）
      long long local_elapsedMicroseconds() const
      {
         return std::chrono::duration_cast<std::chrono::microseconds>(local_total_duration).count();
      }

      // 获取累计的时间（纳秒）
      long long remote_elapsedMicroseconds() const
      {
         return std::chrono::duration_cast<std::chrono::microseconds>(remote_total_duration).count();
      }
   };
   namespace profiling
   {
   ...
   namespace storage
   {
      ...
      public:
         ...
         inline uint64_t getFreeFrames()
         {
            ...
            if (freeFrames == 0)
               throw std::runtime_error("No frames left in free list!");
            return freeFrames;
         }
         ...
         restart:
            ...
            if (bucket_bf.state == BF_STATE::FREE)
            { // important to set it only to free when we have the HT latch or are done with the frame
            ...
            while (*f)
            {
               if ((*f)->pid == pid)
               {
                  throw std::runtime_error("Duplicate pid");
               }
               f = &(*f)->next;
            }
            // -------------------------------------------------------------------------------------
