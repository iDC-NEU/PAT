...
namespace scalestore
{
   namespace storage
   {
            ...
            ssd_fd(ssd_fd)
      {
         ...
         if (n > 0)
         {
            ...
            for (uint64_t t_i = 0; t_i < FLAGS_pageProviderThreads; t_i++)
            {
               ...
               if (t_i == FLAGS_pageProviderThreads - 1)
                  end = n;
               // parititon mailboxes
      ...
      void PageProvider::startThread()
      {
         ...
         auto connect = [&](uint64_t t_i)
         {
            ...
            for (uint64_t n_i = 0; n_i < FLAGS_nodes; n_i++)
            {
               ...
               partitions[t_i].cctxs[n_i].rctx = &(cm.initiateConnection(ip, Type::PAGE_PROVIDER, t_i, bm.nodeId, desport));
         ...
         for (uint64_t t_i = 0; t_i < FLAGS_pageProviderThreads; t_i++)
         {
            pp_threads.emplace_back([&, t_i, connect]() { // ATTENTION: connect needs to be copied other wise leaves scope
               ...
               auto pp_triggered_condition = [&]()
               { return (bm.getFreePages()) < (coolingBFLimit); };
               // epoch condtion
               ...
               // start real eviction
               auto pp_start_eviction_condition = [&]()
               { return (bm.getFreePages() < freeBFLimit); };
               // -------------------------------------------------------------------------------------
               ...
               // -------------------------------------------------------------------------------------
               uint64_t clients =
                   std::accumulate(mhPartitions.begin(), mhPartitions.end(), 0, [](uint64_t a, auto &p)
                                   { return a + p.numberMailboxes; });
               ...
               auto remote_condition = [&](BufferFrame &frame)
               {
                  ...
                  // remote pages
                  if ((frame.pid.getOwner() != bm.nodeId) && (frame.state == BF_STATE::HOT) && !frame.latch.isLatched())
                     return true;
                  return false;
               };
                                                   ...
                                                   std::function<void()> post_process_level)
               {
                  ...
                  while (entries > 0)
                  {
                     ...
                     for (uint64_t b_i = 0; b_i < entries; b_i++)
                     {
                        ...
                        if (tmp_ptr == nullptr)
                           continue; // might happen due to OLC
                  ...
                  if (polled_events > 0)
                  {
                     async_write_buffer.getWrittenBfs(
                         [&](BufferFrame &frame, uint64_t epoch_added)
                         {
                            ...
                            if (epoch_added != frame.epoch.load())
                            {
                               return;
                            }
               ...
               while (threadsRunning)
               {
                  ...
                  for (uint64_t m_i = 0; m_i < partition.numberMailboxes; m_i++)
                  {
                     ...
                     auto endValidPids = std::find_if(inflightCRs.begin(), inflightCRs.end(), [](const PID &pid)
                     ...
                     { // start eviction
                        ...
                        if (FLAGS_evict_to_ssd && sampling_needed)
                        {
                           ...
                           while (samples < required_samples)
                           {
                              ...
                              if (e_sampling_offset > partition_size)
                                 e_sampling_offset = partition_size;
                            ...
                            [&](BufferFrame &frame)
                            {
                               ...
                               if (frame.pid != EMPTY_PID && !frame.latch.isLatched())
                               {
                                  if (FLAGS_evict_to_ssd)
                                  {
                                     ...
                                     {
                                        candidate_batch[candidate_max++] = {frame.epoch, &frame};
                                     }
                                  }
                                  else
                                  {
                                     // we only evict remote pages
                                     if (remote_condition(frame))
                                     {
                                        candidate_batch[candidate_max++] = {frame.epoch, &frame};
                                     }
                                  }
                            ...
                            [&]()
                            {
                               ...
                               for (uint64_t c_i = 0; c_i < candidate_max; c_i++)
                               {
                                  ...
                                  if ((frame.pid.getOwner() == bm.nodeId))
                                  {
                                     ...
                                     if (!rc)
                                        break; // encountered dirty page and we cannot write it to the buffer
                                     continue;
                                  }
                                  // -------------------------------------------------------------------------------------
                                  // Remote Page
                  ...
                  while (!privatePageBuffer.full())
                  { // ensure that the private page Buffer will get refilled again
                     Page *p = nullptr;
                  ...
                  for (uint64_t n_i = 0; n_i < FLAGS_nodes; n_i++)
                  {
                     ...
                     if (partition.cctxs[n_i].responseExpected)
                        continue;
                  ...
                  for (uint64_t m_i = 0; m_i < partition.numberMailboxes; m_i++)
                  {
                     ...
                     if (ctx.readScheduled && !ctx.rdmaReadBuffer.pollCompletion(ctx))
                        continue;
                     auto &response = *ctx.outgoingResp.current;
                     // could be an empty response
                     if (!ctx.responseOutstanding)
                        continue;
                     // -------------------------------------------------------------------------------------
      ...
      void PageProvider::init()
      {
         ...
         {
            if (rContext->type != Type::PAGE_PROVIDER)
               continue;
            // -------------------------------------------------------------------------------------
