...
namespace scalestore
{
   ScaleStore::ScaleStore()
   {
      ...
      if (FLAGS_nodes != 1)
      {
         ...
         {
            if (FLAGS_ownIp == NODES[FLAGS_nodes][nodeId] && FLAGS_port == NODESport[FLAGS_nodes][nodeId])
               break;
         }
      }
      else
      ...
      if (FLAGS_use_proxy)
      {
         while (!(mh->ConnectProxy()))
         {
            printf("wait for connectProxy\n");
            sleep(1);
         }
      }
      printf("scalestore ini over\n");
   }
