...
struct MessageHandler {
   ...
   struct ProxyCctx {
      uintptr_t mbOffset;
      uintptr_t plOffset;
      uintptr_t busyOffset;
      rdma::Message* incoming;
      rdma::Message* outgoing;
      uint8_t *mailbox;
      rdma::RdmaContext* rctx = nullptr;
      uint64_t wqe = 0;
      uint64_t wqe_get_sql = 0;
   };
   // -------------------------------------------------------------------------------------
   ...
   std::vector<ProxyCctx> proxycctxs;
   ProxyCctx cctxForRouterMap;
   std::mutex mtx;
   bool isConnectProxy;
   ...
   bool district_ready = false;
   bool neworder_ready = false;
   bool orderline_ready = false;
   bool customer_update_ready = false;
   bool order_update_ready = false;
   bool stock_update_ready = false;
   bool warehouse_update_ready = false;
       ...
       storage::PartitionedQueue<storage::Page*, PARTITIONS, BATCH_SIZE, utils::Stack>::BatchHandle& page_handle)
   {
      ...
      //-------------------------------------------------------------------------------------
      if(guard.state != STATE::LOCAL_POSSESSION_CHANGE){
         std::cout << static_cast<typename std::underlying_type<STATE>::type>(guard.state) << std::endl;
      }
