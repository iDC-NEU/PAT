...
inline void postRead(void* memAddr, size_t size, ibv_qp* qp, ibv_mr* mr, completion wc, size_t rkey, size_t remoteOffset, size_t wcId  = 0)
{
   struct ibv_send_wr sq_wr;
   struct ibv_sge send_sgl;
   struct ibv_send_wr* bad_wr;
...
class CM
{
  public:
   //! Default constructor
   CM() : port(htons(FLAGS_port)), mbr(FLAGS_dramGB * FLAGS_rdmaMemoryFactor * 1024 * 1024 * 1024), running(true), handler(&CM::handle, this)
   {
      // create thread
      ...
      while (running) {
         ...
         switch (event->event) {
            ...
            case RDMA_CM_EVENT_CONNECT_REQUEST: {
               ...
               context->response = static_cast<RdmaInfo*>(mbr.allocate(sizeof(RdmaInfo)));
            ...
            case RDMA_CM_EVENT_ESTABLISHED: {
               ...
               // std::cout <<"************* received  RKEY << "<<  context->response->rkey << std::endl;
               rdma_ack_cm_event(event);
   ...
   RdmaContext& initiateConnection(std::string ip, Type type, uint64_t typeId, NodeID nodeId,uint64_t desport)
   {
   ...
   RETRY:
      ...
      resolveAddr(outgoingChannel, outgoingCmId, sin, desport);
   ...
   void exchangeInitialMesssage(RdmaContext& context, INITIAL_MSG* initialMessage)
   {
      ...
      while (completions != 2) {
         auto expected = 2 - completions;  // prevents over polling the q and draining it from next phase
         auto comp = pollCompletion(context.id->qp->send_cq, expected,
  ...
  private:
   ...
   uint16_t port;
   utils::SynchronizedMonotonicBufferRessource mbr;  // can we chunk that buffer into sub buffers for clients?
   struct ibv_pd* pd;
   struct ibv_mr* mr;
   // handler section
   ...
   void resolveAddr(rdma_event_channel* outgoingChannel, rdma_cm_id* outgoingCmId, struct sockaddr_storage& sin,uint64_t desport)
   {
      if (sin.ss_family == AF_INET)
         ((struct sockaddr_in*)&sin)->sin_port = htons(desport);
      else
         ((struct sockaddr_in6*)&sin)->sin6_port =htons(desport);
   ...
   void exchangeRdmaInfo(rdma_cm_id* cmId, ibv_mr* mr, Type type, uint64_t typeId, NodeID nodeId)
   {
      ...
      while (completions != 2) {
         auto expected = 2 - completions;  // prevents over polling the q and draining it from next phase
         auto comp = pollCompletion(cmId->qp->send_cq, expected,
                                    wcs);  // assumes that the completion qs are the same for send and recv
         for (int i = 0; i < comp; i++) {
            /* verify the completion status */
            if (wcs[i].status != IBV_WC_SUCCESS) {
   ...
   void createMR()
   {
      ...
      std::cout << "buffer size: " << mbr.getBufferSize() << std::endl;
      mr = ibv_reg_mr(pd, mbr.getUnderlyingBuffer(), mbr.getBufferSize(), IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_WRITE);
      DEBUG_VAR(mr);
      if (!mr)
   ...
   void createQP(rdma_cm_id* cm_id, ibv_cq* completionQueue)
   {
      ...
      if (ret)
         throw std::runtime_error("Could not create QP " + std::to_string(ret) + " errno " + std::to_string(errno));
