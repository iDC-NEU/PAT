...
{
   return scalestore::utils::RandomGenerator::getRand(0, n);
}

// [fromId, toId]
Integer randomId(Integer fromId, Integer toId)
{
   return scalestore::utils::RandomGenerator::getRand(fromId, toId + 1);
}
...
Numeric randomNumeric(Numeric min, Numeric max)
{
   ...
   return min + (scalestore::utils::RandomGenerator::getRandU64() / div);
}
...
void loadWarehouse(Integer from_wh_id, Integer to_wh_id)
{
   ...
   for (Integer i = from_wh_id; i < to_wh_id; i++)
   {
      warehouse.insert({i + 1}, {randomastring<10>(6, 10), randomastring<20>(10, 20), randomastring<20>(10, 20), randomastring<20>(10, 20),
                                 randomastring<2>(2, 2), randomzip(), randomNumeric(0.1000, 0.2000), 3000000, 0}); // last is padding
   }
}
...
void loadStock(Integer w_id)
{
   ...
   for (Integer i = 0; i < ITEMS_NO; i++)
   {
      ...
      stock.insert({w_id, i + 1},
                   {randomNumeric(10, 100), randomastring<24>(24, 24), randomastring<24>(24, 24), randomastring<24>(24, 24),
...
void loadCustomer(Integer w_id, Integer d_id)
{
   ...
   for (Integer i = 0; i < 3000; i++)
   {
      ...
      customer.insert({w_id, d_id, i + 1},
                      {c_first, "OE", c_last, randomastring<20>(10, 20), randomastring<20>(10, 20), randomastring<20>(10, 20),
                       randomastring<2>(2, 2), randomzip(), randomnstring(16, 16), now, c_credit, 50000.00, randomNumeric(0.0000, 0.5000),
                       -10.00, 1, 0, 0, randomastring<500>(300, 500)});
...
void newOrderRnd(Integer w_id)
{
   ...
   for (Integer i = 1; i <= ol_cnt; i++)
   {
      ...
      { // remote transaction
         ...
         // remote_new_order++;
         if (supware <= (Integer)warehouse_range_node.begin || (supware > (Integer)warehouse_range_node.end))
            remote_node_new_order++;
      }
...
int tx(Integer w_id)
{
   ...
   auto end = utils::getTimePoint();
   txn_latencies[transaction_types::NEW_ORDER] += (end - start);
   return 4;
}
