...
void calculate_local(std::string path, int nodes)
{
    // 打开输出文件
    std::string output_filename = path + "local_result";
    std::ofstream output_file(output_filename);
    if (!output_file)
    {
        std::cerr << "无法打开输出文件!" << std::endl;
    ...
    std::vector<std::string> filenames;
    for(int i = 0; i< nodes; i++){
        filenames.push_back(path + "node" + std::to_string(i+1) + "/Logs/local_node" + std::to_string(i) + ".txt");
    }
    ...
    for (const auto &filename : filenames)
    {
        ...
        {
            // 使用正则表达式提取 remote_count 和 increase_count 的值
            std::regex regex(R"(\[.*?\] local_count\s*=\s*(\d+); increase_count\s*=\s*([\d.]+)/s)");
            std::smatch match;
            if (std::regex_match(line, match, regex))
            {
                long long remote_count = std::stoll(match[1].str());
